  ...
  try {
    ...
    if (result.rows[0].role === "admin") {
      return res
        .status(403)
        .json({
          message: "Admin cannot leave the group, assign new admin first",
        });
    }
    await pool.query(
...
const getGroupMembers = async (req, res) => {
  try {
    const group_id = req.params.group_id;
    const result = await pool.query(
      "SELECT users.id, users.username, users.photo, group_members.role FROM group_members JOIN users ON group_members.user_id = users.id WHERE group_members.group_id = $1",
      [group_id]
    );
...
const deleteGroup = async (req, res) => {
  try {
    ...
    if (group.rows[0].admin_id !== user_id) {
      return res
        .status(403)
        .json({ message: "Only the admin can delete this group" });
    }
...
const getUserGroups = async (req, res) => {
  try {
    const user_id = req.user.id;
    const result = await pool.query(
      "SELECT groups.id, groups.admin_id, groups.name, groups.description, group_members.role FROM group_members JOIN groups ON group_members.group_id = groups.id WHERE group_members.user_id = $1",
      [user_id]
    );
    res.json(result.rows);
  } catch (error) {
...
const deleteGroupPost = async (req, res) => {
  ...
  try {
    const post = await pool.query(
      "SELECT * FROM posts WHERE id = $1 AND group_id = $2",
      [post_id, group_id]
    );
