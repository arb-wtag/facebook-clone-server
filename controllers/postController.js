...
const createPost = async (req, res) => {
  try {
    ...
    if (content.length > 500) {
      return res
        .status(400)
        .json({ message: "Post content cannot exceed 500 characters" });
    }
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        const compressedPath = `uploads/compressed-${Date.now()}-${
          file.filename
        }.jpg`;
    ...
    const result = await pool.query(
      "INSERT INTO posts (user_id, group_id, content, image) VALUES ($1, $2, $3, $4) RETURNING *",
      [
        user_id,
        group_id || null,
        content,
...
const getUserPosts = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT posts.*, users.username, users.photo AS user_photo, groups.name AS group_name FROM posts JOIN users ON posts.user_id = users.id LEFT JOIN groups ON posts.group_id = groups.id WHERE posts.user_id = $1 ORDER BY posts.created_at DESC",
      [user_id]
    );
    res.json(result.rows);
  } catch (error) {
...
const getGroupPosts = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT posts.*, users.username, users.photo AS user_photo FROM posts JOIN users ON posts.user_id = users.id WHERE posts.group_id=$1 ORDER BY posts.created_at DESC",
      [group_id]
    );
    res.json(result.rows);
  } catch (error) {
